...
| 页面发布/子资源时序 | Tested | 浏览器集成测试（轮询等待辅助） | **先发布后加载**：HTML 解析+样式+字体+脚本完成后立即发布页面，`<img>/<video>` 在线程池后台抓取注入（Page 版本失效驱动 50ms 周期重绘）；单个失败 CDN 请求不再阻塞首屏（实测百度 DOM 就绪 12s→1.7s）。@font-face 跨加载趟去重（HasWebFont 查询跳过重复抓取） |
