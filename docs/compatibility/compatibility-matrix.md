...
| 动态 `import()` | Partial | 2 JS 单元测试 + 2 浏览器集成测试 | classic 与模块代码中均可用：classic 脚本以文档 URL 求值，相对 specifier 对页面解析；返回真 Promise（微任务泵结算，定时器/事件回调内同样可用）；加载失败 reject；与静态 import 共享模块缓存（同 URL 只抓取一次）；top-level await 实测可用。无 import maps、classic 中 `import.meta` 为运行时 TypeError（浏览器为解析错误） |
