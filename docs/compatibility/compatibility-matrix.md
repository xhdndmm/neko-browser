...
| 合成器（Compositor） | Tested | 17 合成器单元测试 + UI 冒烟/截图 | **软件合成器抽象（ADR 0015）**：`Surface`（RGBA8888 拷贝/alpha 混合/滚动原语）+ `Compositor` 接口（输出表面 + 有序图层、全量/脏矩形 `Composite( Rect)`、`ScrollOutput` 报告暴露带）；混合数学与 Rasterizer 一致；GUI 已接线（图层 0 = 光栅化页面，图层 1 = caret 覆盖层，闪烁/移动走脏矩形重合成，滚动为带级 blit）；**GPU 后端尚未实现** |
...
| 多进程 / IPC | Partial（M1） | 12 IPC 单元测试 + 9 协议单元测试 + 1 端到端子进程测试 | **ADR 0016 M1**：`neko::ipc` 帧协议 Channel（u32 长度前缀、64 MiB 帧上限、POSIX 管道/Windows 匿名管道）+ `Subprocess`（fork+exec / CreateProcess）；**Renderer 子进程**（`--renderer-child`）在独立地址空间跑完整页面管线（fetch→parse→style→layout→rasterize），经 `RendererHost` 回传位图 + DOM/标题（协议带边界检查）；CLI `--renderer-process` 路由加载/截图/dump-dom 到子进程；子进程崩溃不再带崩浏览器（错误经 IPC 返回）；**尚未**：GUI 接入（M2）、Network/GPU 进程（M3/M4）、沙箱与站点隔离（M5）、每站点会话复用（现每次加载新建子进程） |
