...
| ES 模块（`<script type="module">`） | Partial | 8 JS 单元测试 + 3 浏览器集成测试 | inline 与外部 src 均按 ES 模块求值（自有顶层作用域、严格模式）；defer 相位执行（async 属性进 async 相位）；静态 import 经模块加载器走页面网络路径（带 Cookie，HTTP≥400 视为加载失败）；specifier 解析：绝对 URL 直用、`./`/`../`/`/` 相对导入模块自身 URL、**裸 specifier 报错**（无 import map）；import.meta.url 已填充；同 URL 只抓取/求值一次；循环依赖按 ESM 语义；document.currentScript 在模块中为 null。无动态 import()、无 import maps、无 top-level await 验证 |
