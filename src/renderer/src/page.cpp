...
bool Page::AdvanceAnimations()
{
  ...
  for (auto& [element, state] : animation_states_) {
    ...
    // The schedule itself lives in the image module so that a directly
    // navigated GIF (browser) plays exactly the same way.
    if (!image::AdvanceGifFrame(
            *state.animation, now_ms - state.start_ms, state.frame, state.loops, state.finished)) {
      continue;
    }
    ...
    const image::GifFrame& current = state.animation->frames[state.frame];
    const auto image_it = images_.find(element);
    if (image_it != images_.end() && image_it->second.rgba.size() == current.rgba.size()) {
      std::memcpy(image_it->second.rgba.data(), current.rgba.data(), current.rgba.size());
      display_list_.reset();
      BumpVersion();
      changed = true;
    }
