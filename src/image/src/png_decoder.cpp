...
base::Result<PngInfo> ParseIHDR(std::string_view data)
{
  ...
  const bool valid_depth =
      (info.color_type == 0 && (info.bit_depth == 1 || info.bit_depth == 2 || info.bit_depth == 4 ||
                                info.bit_depth == 8 || info.bit_depth == 16)) ||
      (info.color_type == 2 && (info.bit_depth == 8 || info.bit_depth == 16)) ||
      (info.color_type == 3 && (info.bit_depth == 1 || info.bit_depth == 2 || info.bit_depth == 4 ||
                                info.bit_depth == 8)) ||
...
// bytes-per-pixel.
base::Result<void>
Unfilter(std::string_view raw, int width_bytes, int bpp, int height, std::vector<uint8_t>& out)
{
  ...
  for (int y = 0; y < height; ++y) {
    const uint8_t* row =
        reinterpret_cast<const uint8_t*>(raw.data()) + static_cast<size_t>(y) * stride;
    ...
    for (int i = 0; i < width_bytes; ++i) {
      ...
      case 2:
        ...
        break; // Up
      case 3:
        value = raw_byte + (a + b) / 2;
        break; // Average
      case 4:
        value = raw_byte + PaethPredictor(a, b, c);
        break; // Paeth
      default:
...
void ConvertRow(const PngInfo& info,
                const uint8_t* row,
                int pixel_width,
                int offset_x,
                int xstep,
                ...
                std::vector<uint8_t>& rgba)
{
  ...
  uint8_t* dst =
      rgba.data() + (static_cast<size_t>(offset_y) + static_cast<size_t>(y) * ystep_u) * stride;
  ...
  for (int x = 0; x < pixel_width; ++x) {
    ...
    case 0: { // grayscale
      ...
      if (!info.trns_alpha.empty()) {
        const bool transparent = info.bit_depth == 16
    ...
    case 2: { // RGB
      ...
      if (info.trns_alpha.size() >= 6 && r == info.trns_alpha[0] && g == info.trns_alpha[2] &&
          b == info.trns_alpha[4]) {
        a = 0;
      }
      break;
...
base::Result<void> DecodeBlock(const PngInfo& info,
                               std::string_view raw,
                               int block_width,
                               int block_height,
                               int offset_x,
                               ...
                               std::vector<uint8_t>& rgba)
{
  ...
  for (int y = 0; y < block_height; ++y) {
    ConvertRow(info,
               recon.data() + static_cast<size_t>(y) * static_cast<size_t>(packed_row_bytes),
               block_width,
               offset_x,
               xstep,
               offset_y,
...
base::Result<Image> DecodePng(std::string_view data)
{
  ...
  while (pos + 12 <= data.size()) {
    ...
    const uLong crc = crc32(
        0L, reinterpret_cast<const Bytef*>(data.data() + pos + 4), static_cast<uInt>(4 + length));
    if (crc != expected_crc) {
    ...
    if (type == kChunkIHDR) {
      if (info.has_value())
        return base::Error::Parse("png: duplicate IHDR");
      ...
      if (!info.has_value())
        return base::Error::Parse("png: PLTE before IHDR");
      if (length % 3 != 0)
        return base::Error::Parse("png: bad PLTE length");
      info->palette.clear();
      for (size_t i = 0; i < length; i += 3) {
        info->palette.push_back(PaletteEntry{static_cast<uint8_t>(chunk[i]),
                                             static_cast<uint8_t>(chunk[i + 1]),
                                             static_cast<uint8_t>(chunk[i + 2])});
      }
    } else if (type == kChunkTRNS) {
      if (!info.has_value())
        return base::Error::Parse("png: tRNS before IHDR");
  ...
  if (!info.has_value())
    return base::Error::Parse("png: missing IHDR");
  if (idat.empty())
    return base::Error::Parse("png: missing IDAT");
  ...
  if (p.interlace == 0) {
    ...
    auto r = DecodeBlock(
        p, std::string_view(zdata).substr(0, needed), p.width, p.height, 0, 1, 0, 1, out.rgba);
    if (!r)
      return r.error();
    consumed = needed;
  } else {
    for (const Adam7Pass& pass : kAdam7) {
      ...
      const size_t needed = static_cast<size_t>(ph) * (static_cast<size_t>(packed_row_bytes) + 1);
      if (consumed + needed > zdata.size()) {
        return base::Error::Parse("png: truncated Adam7 pass data");
      }
      auto r = DecodeBlock(p,
                           std::string_view(zdata).substr(consumed, needed),
                           pw,
                           ph,
                           pass.xstart,
                           pass.xstep,
