...
Error Error::Io(std::string message) { return Error(ErrorCategory::kIo, std::move(message)); }
...
ErrorCategory Error::category() const { return category_; }
const std::string& Error::message() const { return message_; }
bool Error::ok() const { return category_ == ErrorCategory::kNone; }
Error::operator bool() const { return category_ != ErrorCategory::kNone; }

bool operator==(const Error& lhs, const Error& rhs) {
  return lhs.category() == rhs.category() && lhs.message() == rhs.message();
}

bool operator!=(const Error& lhs, const Error& rhs) { return !(lhs == rhs); }

BadResultAccess::BadResultAccess(const Error& error)
    : std::runtime_error("bad result access: [" +
                         std::string(ToString(error.category())) + "] " + error.message()),
      category_(error.category()) {}

ErrorCategory BadResultAccess::category() const { return category_; }
